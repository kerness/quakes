...
class QuakeList(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if lat and lng and radius is not None:
            self.logger.debug(
                f"21Given parameters: lat: {lat}, lng: {lng}, radius: {radius}"
            )
            ...
            self.logger.debug(
                f"Given parameters: lat: {lat}, lng: {lng}, radius: {radius}"
            )
            point = Point(float(lat), float(lng), srid=4326)
            queryset = Quake.objects.filter(
                geom__distance_lte=(point, D(km=float(radius)))
            )
